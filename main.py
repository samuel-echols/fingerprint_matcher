...
# Compare each unique pair of fingerprints
for i, (en_fingerprint_1, ridge_endings_1, bifurcations_1) in enumerate(fingerprint_data):
    for j, (en_fingerprint_2, ridge_endings_2, bifurcations_2) in enumerate(fingerprint_data):
        if i >= j:
            continue
        matches = d_match.match(en_fingerprint_1, ridge_endings_1, bifurcations_1, en_fingerprint_2, ridge_endings_2, bifurcations_2, view=False)
        ...
        print(f'Match between fingerprint {i + 1} and fingerprint {j + 1}: {len(matches[0]) + len(matches[1])} matches,')
